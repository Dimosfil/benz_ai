export const TELEGRAM_BOT_PROFILE = {
  ...
  description: [
    ...
    "Отправьте город, район или регион обычным сообщением.",
    "Примеры: Воронеж; Коминтерновский район, Воронеж; Новоусманский район, Воронежская область.",
    "Подробная справка — /help.",
  ].join("\n"),
  shortDescription: "Поиск АЗС и вероятностного наличия топлива по городу, району или региону России.",
};

const HELP_TEXT = [
  "⛽ Benz AI ищет АЗС и вероятностные данные о наличии топлива.",
  "Просто отправьте название территории обычным сообщением.",
  [
    "Примеры запросов:",
    "• город: Воронеж",
    "• район города: Коминтерновский район, Воронеж",
    "• район области: Новоусманский район, Воронежская область",
    "• населённый пункт: Бабяково, Воронежская область",
  ].join("\n"),
  "Если название встречается в разных регионах, добавьте область или край.",
  "Чтобы заново запросить данные у источников, отправьте /refresh и территорию, например: /refresh Новая Усмань.",
  "Данные о наличии вероятностные. Веб-версия показывает полный список станций, фильтры и источники.",
].join("\n\n");
...
export function createBenzTelegramHandler({ findSummary, refreshSummary = findSummary, buildInfo = null }) {
  return async function handleTelegramMessage(message) {
    ...
    if (/^\/(start|help)(?:@\w+)?$/i.test(text)) {
      return appendBuildInfo(HELP_TEXT, buildInfo);
    }
