...
## Проверка банковских API от 25 сентября 2026

- Подробные ответы, сертификат и план исправления:
  [исследование](../../../../docs/bank-api-investigation-2026-09-25.md).
- Альфа и T-Bank доступны по прежним URL после добавления официального
  Russian Trusted Root CA в отдельный проверочный HTTPS-клиент. Без него
  воспроизведён `SELF_SIGNED_CERT_IN_CHAIN`; проверка TLS не отключалась.
- Альфа вернула цены Газпрома в Бабяково и Лукойла на Цветочной, 26.
  T-Bank вернул свежие оплаты и `yandexOrgId`; `priceByFuelType` у этих двух
  станций пустое, схема непустого значения пока не подтверждена.
- Сбер вернул прямой JSON без cookies/Chromium. Обязательная проверка
  `__jhash_` и `КАРТА ЗАПРАВОК` в нашем worker устарела. В ответе Сбера нет цен;
  оплата Лукойла есть, у двух записей Газпрома на совпадающих координатах её нет.
- По результатам исследования реализованы scoped CA для двух банков и прямой
  HTTP-запрос Сбера с браузерным резервом. Локальный `/api/stations` подтвердил
  доступность всех трёх источников, цены Альфы и банковские оплаты двух станций.
  Публикация и проверка BotHost — отдельный этап. Исторические проверки ниже
  описывают состояние источников на свои даты.
